...
public class QiMenZhiRun extends QiMen {


//*******************************************************************************************************************************
    public QiMenZhiRun(QiMenZhuanPanJiChuSetting qiMenZhuanPanJiChuSetting) {
    ...
    private void initializeFuTouAndJieQi() {
        ...
        if (this.qiMenZhuanPanJiChuSetting.getPaiPanType() == 3) {
            // 2.1、计算符头（根据日干寻找最近的甲或己）
            // 这是计算“当天所属的符头”，用于确定三元
            int ganIndex = -1;
            String[] tianGan = {"甲", "乙", "丙", "丁", "戊", "己", "庚", "辛", "壬", "癸"};
            for (int i = 0; i < tianGan.length; i++) {
                if (tianGan[i].equals(this.dayGan)) {
                    ganIndex = i;
                    break;
                }
            }
            // 如果是甲~戊(0-4)，符头是当前的索引值天数前；如果是己~癸(5-9)，符头是当前索引-5天前
            int offset = (ganIndex < 5) ? ganIndex : (ganIndex - 5);

            // 获取符头的公历和农历对象
            Solar fuTouSolar = this.solar.next(-offset);
            Lunar fuTouLunar = fuTouSolar.getLunar();
            this.fuTou = fuTouLunar.getDayInGanZhiExact(); // 设置符头（如：甲子）

            // 2.2、判定节气（置闰法核心：寻找距离下一个节气最近的符头作为切换点）
            // 逻辑修正：不再简单判断当前符头与下一节气的距离，而是计算“下一节气真正的符头切换点”

            // 获取下一个节气信息
            JieQi nextJieQi = this.lunar.getNextJieQi(true);
            Solar nextJieSolar = nextJieQi.getSolar();
            String nextJieQiName = nextJieQi.getName();

            // 获取下一个节气当天的天干
            Lunar nextJieLunar = nextJieSolar.getLunar();
            String nextDayGan = nextJieLunar.getDayGan();

            // 计算下一个节气当天的天干索引
            int nextGanIndex = -1;
            for (int i = 0; i < tianGan.length; i++) {
                if (tianGan[i].equals(nextDayGan)) {
                    nextGanIndex = i;
                    break;
                }
            }
            ...
            // 计算距离下一个节气最近的符头（甲/己）
            // 规则：看节气当天离 前一个甲/己 近，还是离 后一个甲/己 近。
            // 离谁近，就以谁作为换气的起始点（符头）。
            int distToPrev = nextGanIndex % 5; // 距离前一个甲/己的天数
            int distToNext = 5 - distToPrev;   // 距离后一个甲/己的天数

            Solar switchSolar;
            if (distToPrev <= distToNext) {
                // 距离前一个符头更近（超神），切换点为前一个符头
                switchSolar = nextJieSolar.next(-distToPrev);
            } else {
                // 距离后一个符头更近（接气），切换点为后一个符头
                switchSolar = nextJieSolar.next(distToNext);
            }

            // 比较当前排盘日期与“节气切换日期”
            // 使用 LocalDate 进行比较，避免 Date/Calendar 的时分秒干扰
            LocalDate currentDate = LocalDate.of(this.solar.getYear(), this.solar.getMonth(), this.solar.getDay());
            LocalDate switchDate = LocalDate.of(switchSolar.getYear(), switchSolar.getMonth(), switchSolar.getDay());

            if (currentDate.isBefore(switchDate)) {
                // 如果当前日期在切换点之前，使用上一个节气
                this.jieQi = this.lunar.getPrevJieQi(true).getName();
            } else {
                // 如果当前日期已经到了切换点或之后，使用下一个节气
                this.jieQi = nextJieQiName;
            }
        ...
        if (this.qiMenZhuanPanJiChuSetting.getPaiPanType() == 2) {
            ...
            for (int i = 0; i < sanYuanFuTou.length; i++) {
                ...
                if (dayGanZhi.equals(sanYuanFuTou[i]) && !"".equals(this.lunar.getJieQi())) {
                    ...
                    this.jieQi = this.lunar.getJieQi();
                    return;
                }
                // 情况二：当天日干支是符头、但当天不是节气
                if (dayGanZhi.equals(sanYuanFuTou[i]) && "".equals(this.lunar.getJieQi())) {
                    this.fuTou = dayGanZhi;
                    this.jieQi = this.lunar.getPrevJieQi(this.qiMenZhuanPanJiChuSetting.getJieQiType() == 0).getName();
                    return;
                }
                // 情况三：当天日干支不是符头、当天是节气
                if (!dayGanZhi.equals(sanYuanFuTou[i]) && !"".equals(this.lunar.getJieQi())) {
                    String jieQi = this.lunar.getPrevJieQi(this.qiMenZhuanPanJiChuSetting.getJieQiType() == 0).getName();
                ...
                // 情况四：当天日干支不是符头、当天不是节气
                if (!dayGanZhi.equals(sanYuanFuTou[i]) && "".equals(this.lunar.getJieQi())) {
                    dayGanZhi = (this.qiMenZhuanPanJiChuSetting.getDayGanZhiType() == 1) ? this.lunar.next(-i).getDayInGanZhiExact() : this.lunar.next(-i).getDayInGanZhiExact2();
                    String jieQi = this.lunar.next(-i).getJieQi();
                    for (String key : sanYuanFuTou) {
    ...
    private void initializeSanYuanAndJuShu() {
        ...
        if (this.qiMenZhuanPanJiChuSetting.getPaiPanType() == 0) {
            Map<Integer, List<Object>> sanYuanDateRangeMap = QiMenZhuanPanJiChuMap.SAN_YUAN_DATE_RANGE;
            for (int i = 0; i < sanYuanDateRangeMap.size(); i++) {
                List<Object> list = sanYuanDateRangeMap.get(i);
                int startDate = Integer.parseInt(list.get(0).toString());
                int endDate = Integer.parseInt(list.get(1).toString());
        ...
        if (this.qiMenZhuanPanJiChuSetting.getPaiPanType() == 1) {
            ...
            if (this.qiMenZhuanPanJiChuSetting.getYueJiaQiJuType() == 0) {
                ...
                diZhi = QiMenZhuanPanJiChuMap.SIX_JIA_ZI_XUN_SHOU_AND_YI_ZHANG.get(this.yearGanZhi).get(0).substring(1, 2);
            }
        ...
        // 3、日家奇门
        if (this.qiMenZhuanPanJiChuSetting.getPaiPanType() == 2) {
            Map<String, String> riZhuFuTouMap = QiMenZhuanPanJiChuMap.RI_ZHU_SAN_YUAN;
            ...
            if (day <= 60) this.juShu = 1;
            if (day > 60 && day <= 120) this.juShu = 7;
            if (day > 120 && day <= 180) this.juShu = 4;
            if (day > 180 && day <= 240) this.juShu = 9;
            if (day > 240 && day <= 300) this.juShu = 3;
            if (day > 300) this.juShu = 6;
            return;
        }
        ...
        if (this.qiMenZhuanPanJiChuSetting.getPaiPanType() == 3) {
            // 4.1、计算三元
            // 置闰法中，三元由符头决定（子午卯酉为上元，寅申巳亥为中元，辰戌丑未为下元）。
            // 因RI_ZHU_SAN_YUAN是基于60甲子顺序排列的，且符头甲子、甲午、己卯、己酉均为上元，所以直接用日干支查表亦可。
            this.sanYuan = QiMenZhuanPanJiChuMap.RI_ZHU_SAN_YUAN.get(this.fuTou);

            // 4.2、计算局数
            // 使用在上一步(initializeFuTouAndJieQi)中经过超神/接气逻辑判定后的【this.jieQi】
            List<Integer> juShu = QiMenZhuanPanJiChuMap.JU_SHU.get(this.jieQi);
...
//-------------------------------------------------------------------------------------------------------------------------------
    ...
    // ==========================================================================================
    // 适配 Compose UI 的数据模型与输出函数
    // ==========================================================================================


    /**
     * 获取未包含在数据模型中的详细排盘信息（格式化字符串）
     * 用于在 UI 的“详情”或“分析”弹窗中显示
     * @return 格式化良好的多行字符串
     */
    public String getFormattedDebugInfo() {
        ...
        sb.append("## 基础信息\n");
        sb.append("符头：").append(getFuTou()).append("  ");
        sb.append("三元：").append(getSanYuan()).append("\n");
        sb.append("旬首：").append(getXunShou()).append(" (仪仗: ").append(getXunShouYiZhang()).append(")\n");
        sb.append("三乙：天乙[").append(getTianYi()).append("]  地乙[").append(getDiYi()).append("]  太乙[").append(getTaiYi()).append("]\n");
        sb.append("出生节气：").append(getChuShengJie()).append("\n");
        sb.append("出生二气：").append(getChuShengQi()).append("\n");
        sb.append("\n");

        sb.append("## 吉凶格局\n");

        // 1. 伏吟反吟
        List<String> fuYin = getFuYin();
        List<String> fanYin = getFanYin();
        if (!fuYin.isEmpty()) sb.append("伏吟：").append(String.join("、", fuYin)).append("\n");
        if (!fanYin.isEmpty()) sb.append("反吟：").append(String.join("、", fanYin)).append("\n");

        // 2. 六仪击刑与入墓
        List<String> jiXing = getLiuYiJiXing();
        List<String> ruMu = getQiYiRuMu();
        if (!jiXing.isEmpty()) sb.append("击刑：").append(jiXing).append("\n");
        if (!ruMu.isEmpty()) sb.append("入墓：").append(ruMu).append("\n");

        // 3. 门迫
        List<String> menPo = getMenPoStatus();
        boolean hasMenPo = false;
        StringBuilder menPoSb = new StringBuilder();
        for(int i=0; i<menPo.size(); i++) {
            if(menPo.get(i) != null && !menPo.get(i).isEmpty()) {
                menPoSb.append(menPo.get(i)).append(" ");
                hasMenPo = true;
            }
        }
        if(hasMenPo) sb.append("门迫：").append(menPoSb).append("\n");
        sb.append("\n");

        sb.append("## 奇门九遁\n");
        List<List<String>> jiuDun = getJiuDun();
        boolean hasDun = false;
        for (int i = 0; i < jiuDun.size(); i++) {
            List<String> duns = jiuDun.get(i);
            if (!duns.isEmpty()) {
                sb.append(QiMenZhuanPanJiChuMap.JIU_GONG_GONG_WEI.get(i)).append("：").append(duns).append("\n");
                hasDun = true;
            }
        }
        if (!hasDun) sb.append("本局无九遁格局\n");
        sb.append("\n");

        sb.append("## 内外盘判断\n");
        List<String> neiWai = getNeiWaiGongWeiInfo();
        for (String info : neiWai) {
            sb.append(info).append("\n");
        }
        sb.append("\n");

        sb.append("## 十干/八门克应\n");
        // 这里只简单列举，防止字符串过长，实际应用可按宫位详细展开
        sb.append(getShiGanKeYing());
